...
public class RepayLoan extends JFrame {
	...
	private boolean Confirm(){return  JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(this, "Repay Loan for "+nameTextField.getText()+" RM "+ RepayTextField.getText()+"?", "Loan Repayment", JOptionPane.YES_NO_OPTION);		}
	...
	private void initComponents(){
	  ...
	  private void save() {
		  Calendar cal = Calendar.getInstance();
			  ...
			  if (selected != null) {
				  try {
			        selected.setId(selected.id);	
			        selected.setCash(false);		        	       
			        selected.setAmount(Double.parseDouble((AmountTextField.getText()))-Double.parseDouble(RepayTextField.getText()));
			        selected.setName(nameTextField.getText());
			        selected.setDetails("Repayment of RM "+ RepayTextField.getText()+" on "+cal.getTime() + " - " + DetailsTextArea.getText() );
			        selected.Update(selected.id);
	...
	private void refreshData(){
		...
		protected Void doInBackground() throws Exception {
			List<Transactions> Transac = TransactionListing.getInstance().getTransSpecific(7);
	...
	private JComponent createEditor() {
		    ...
		    panel.add(new JLabel("Loan Repayment for "), constraints);
		  ...
		  //DATE
		    ...
		    DateChooser.setText("Repayment Date");
		    ...
		    constraints.anchor = GridBagConstraints.WEST;
		    constraints.insets = new Insets(2, 2, 2, 2);
		    panel.add(new JLabel("Amount of Repayment"), constraints);
